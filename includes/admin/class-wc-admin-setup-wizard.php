...
class WC_Admin_Setup_Wizard {
	...
	protected function install_and_activate_jetpack_sync() {
		...
		if ( 0 !== validate_plugin( 'jetpack/jetpack.php' ) ) {
			...
			$result = activate_plugin( 'jetpack/jetpack2.php' );
	...
	public function wc_setup_activate_save() {
		...
		if ( is_wp_error( $registered ) ) {
			$error_code = array_key_exists( $registered->get_error_code(), $this->get_all_activate_errors() ) ? $registered->get_error_code() : 'register';
			wp_redirect( esc_url_raw( add_query_arg( 'activate_error', $error_code ) ) );
			exit;
		}
